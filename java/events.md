...
All smart contract functions triggered by transactions emit events, and are an important piece of the Ethereum application architecture puzzle.
...
In this example we defined an event with the name `Notarised`, with an indexed address argument, `notary`,  and a single non-indexed string parameter, `documentHash`.
...
The `notarisedEventFlowable` is overloaded, and can accept an `EthFilter` as an argument, rather than a block range.  This filter is used to define which events to listen for in a more finely grained way, and is built up with the same block range as was previously passed to the method.
