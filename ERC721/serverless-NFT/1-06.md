...
Also don't forget to set the deploy parameters for the token. I'm just using "Token" for the name and "TKN" for the symbol but these can be whatever you want. The third deploy parameter is the metadata address which you can get from the previously deployed metadata contract with `metadata.address`.
